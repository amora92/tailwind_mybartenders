...
const EditArticle = () => {
  ...
  const addTag = () => {
    ...
    if (trimmedTag && !article.tags.includes(trimmedTag)) {
      setArticle(prev => ({ ...prev, tags: [...prev.tags, trimmedTag] }))
      setTagInput('')
    }
  }
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      const newArticle = {
        ...
        contentSections,
        tags: article.tags,
        status: 'draft' as const, // New copies start as drafts
        author: {
          name: 'MyBartenders',
      ...
      setSuccess('Article duplicated! Redirecting to the new article...')
      setTimeout(() => {
        router.push(`/admin/edit-article/${newSlug}`)
      }, 1500)
    } catch (err) {
        ...
        <div className='flex flex-col md:flex-row md:items-center md:justify-between mb-8'>
          ...
          <div className='mt-4 md:mt-0 flex items-center gap-3'>
            ...
            <button
              ...
              disabled={copying}
              className='inline-flex items-center gap-2 px-4 py-2 bg-indigo-500/20 border border-indigo-500/30 text-indigo-400 hover:bg-indigo-500/30 rounded-xl transition-all disabled:opacity-50'
            >
              {copying ? (
                <>
                  ...
                  <div>
                    ...
                    <p className='mt-2 text-xs text-gray-500'>
                      {article.status === 'draft'
                        ...
                        : 'This article is live and visible to everyone.'}
                    </p>
                  </div>
                  ...
                  >
                    {saving ? (
                      <>
                        ...
                        {article.status === 'draft' ? 'Saving draft...' : 'Saving...'}
                      </>
                    ) : pendingUploads > 0 ? (
                      <>
                        <svg className='w-5 h-5 animate-spin' fill='none' viewBox='0 0 24 24'>
                  ...
                  <div>
                    ...
                    {seoAssessment.recommendations.length > 0 ? (
                      ...
                      <p className='rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-sm text-gray-300'>
                        This article already looks healthy from an on-page SEO perspective.
                      </p>
                    )}
                  </div>
                </div>
